...
```ts
declare function createGame(red: boolean, green: boolean, yellow: boolean, blue: boolean): IGameState|null;
```

Creates a new `IGameState`-Object. Thus, it generates a new game in the starting configuration.
...
```ts
declare function getIndexOfPawnAtPosition(game: IGameState, position: IPosition): number;
```
...
```ts
declare function getMoves(game: IGameState, pawnIndex: number): IPosition[];
```
...
```ts
declare function makeMove(game: IGameState, pawnIndex: number, destination: IPosition): IGameState|null;
```
...
```ts
declare function makeComputerMove(game: IGameState): IGameState;
```
...
```ts
declare function arePlayersAlive(game: IGameState): {[player in EColor]: boolean};
```
...
```ts
declare function isGameOver(game: IGameState): boolean;
```
...
```ts
declare function isGame(game: any): game is IGameState;
```
